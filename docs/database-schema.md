...
- PRIMARY KEY (`user_uid`, `course_id`, `lesson_id`)
- FOREIGN KEY (`user_uid`) REFERENCES `users`(`uid`) ON DELETE CASCADE
- FOREIGN KEY (`course_id`) REFERENCES `courses`(`id`) ON DELETE CASCADE
...
### `user_activity_events` Table (This table will now be in MongoDB)
- **Note:** This table/collection will be moved to MongoDB for better flexibility with event data structures.
  See "Proposed MongoDB Schema" section below.
...
## Proposed MongoDB Schema (NoSQL)

This section outlines proposed collections for MongoDB, suitable for data that is less structured or benefits from document-based storage.

### `user_activity_logs` (MongoDB Collection)
Stores a stream of user activities on the platform. This is a good candidate for MongoDB due to potentially varied event structures.
- `_id`: ObjectId (Primary Key - automatically generated by MongoDB)
- `user_uid`: STRING (Indexed, references `users.uid` in MySQL)
- `event_type`: STRING (e.g., 'course_enrollment', 'project_created', 'lesson_completed', 'forum_post', 'comment_added', 'badge_earned')
- `event_data`: OBJECT (Flexible object to store event-specific data)
  - `course_id`: STRING (Optional, if event_type is course-related)
  - `project_id`: STRING (Optional, if event_type is project-related)
  - `lesson_id`: STRING (Optional)
  - `points_earned`: NUMBER (Optional)
  - `custom_fields`: OBJECT (Any other relevant data for the event)
- `timestamp`: Date (ISODate, Indexed - time of the event)
- `ip_address`: STRING (Optional)
- `user_agent`: STRING (Optional)

**Example `event_data` for 'course_enrollment':**
```json
{
  "course_id": "course_abc_123",
  "course_title": "Introduction to Solidity",
  "enrollment_source": "course_page_button"
}
```

**Example `event_data` for 'badge_earned':**
```json
{
  "achievement_id": "COURSE_COMPLETION_MASTER",
  "badge_name": "Course Completion Master",
  "points_earned": 50
}
```

### `dashboard_recommendations_cache` (MongoDB Collection - Optional)
Could be used to cache AI-generated dashboard recommendations for users to reduce load on Genkit flows.
- `_id`: STRING (user_uid)
- `recommendations`: ARRAY of OBJECTS (The structure from `DashboardRecommendationsOutputSchema`)
- `explanation`: STRING
- `generated_at`: Date
- `expires_at`: Date (Indexed, for TTL)
...
Using MongoDB for activity logs allows for easier schema evolution as new types of events are added, without requiring ALTER TABLE statements. It's also well-suited for high write throughput.
